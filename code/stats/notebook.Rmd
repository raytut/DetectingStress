...
# Theme 2
ptheme <- theme(text=element_text(size=11), 
...
ggsave("figures/fig_1A_SubjectiveStress_Week.svg", device="svg", units="in", width=4, height=4, dpi=320, bg="transparent")
```
...
ggsave("figures/fig_1B_Outcomes.svg", device="svg", units="in", width=4, height=4, dpi=320, bg="transparent")
```
...
ggsave("figures/fig_2A_Event.svg", device="svg", units="in", width=4, height=4, dpi=320, bg="transparent")
...
ggsave("figures/fig_2B_Activity.svg", device="svg", units="in", width=4, height=4, dpi=320, bg="transparent")
...
ggsave("figures/fig_2C_Social.svg", device="svg", units="in", width=4, height=4, dpi=320, bg="transparent")
...
ggsave("figures/fig_2C_Physical.svg", device="svg", units="in", width=4, height=4, dpi=320, bg="transparent")
# All
ggarrange(plot.event_substr, plot.activity_substr, plot.social_substr, plot.physical_substr, common.legend = T, legend="right")
ggsave("figures/fig_2_All.svg", device="svg", units="in", width=7, height=7, dpi=320, bg="transparent")
```
...
ggsave("figures/fig_MoodkResid_ALL.svg", device="svg", units="in", width=4, height=4, dpi=320, bg="transparent")
```
...
In order to perform cross-validation, we try out two methods: leave-one-beep-out (LOBO), and leave-one-subject-out (LOSO) analysis. For the LOBO, we train a random forest model for each subject separately (so 81 subjects=81 models), where we leave out one survey (or beep) when running the model. We then test the model on the beep that was left out. We repeat this process for the subject until we've removed each survey/beep out once. This results in a series of predictions that give us an error level for each subject. In order to do this, we have developed a function that is provided in this github directory mentioned in the introducton. This will produce the classification errors for each subject. 

We additionally want to test the generalizability of the models. If we get all this data, can we use it on a novel participant? So to this end, we use the LOSO model where we remove a participants' entire data set, train our model on the remaining data, and then test it on the subject that was left out. This gives us a confusion matrix from which we calculate the error rates for these models.

Finally, we also need to test all our models against the  true chance levels. One can assume in theory that the prediction error for a dichotomous variable is 50/50, but we dont like to do things the easy way. Instead, we will do 10000 iterations where we bootstrap resample the stress and control weeks. This will give us the real error rate with a sampling distribution. This was made into a function, but for some reason I couldnt get the function to run in parallel processing on our unix system. So instead I run the parallel processing of the bootstrap outside of a function. Its not pretty, but it works.


### Model 1: Mood {.tabset}

Here we estimate our models based on mood only. Using mood, we try to classify which week participants were in. First we run the bootstrap, and then the LOSO and LOBO models. We see that the mean LOBO error rate is 33.76 percent, and that the model performs relatively well on most subjects. The LOSO model does considerably worse with around 45% error rates. Our bootstrap shows that its not exactly 50% chance, but it is close enough at least. We will still use the boot strap analysis later
...
Subject level p-values
```{r}
loso.mood_null <- rf.null_test(forest.loso.mood, forest.boot.mood.merge, model_type="LOSO", method="actual")
loso.mood_null$p_val
```
...
The second model I will test is whether we can predict week type from the physiology data alone. For this model, the LOBO error rate is 36.11, which is still better than chance level predictions, but worse than model 1.The LOSO errors here are also pretty high. We will later test them empirically
...
*Subject level p-values:*
```{r message=FALSE, warning=FALSE}
loso.physio_null <- rf.null_test(forest.loso.physio, forest.boot.physio.merge, model_type="LOSO", method="actual")
loso.physio_null$p_val
```
...
Finally we check the full model using both physio and mood variables. This model perfroms better than the mood model. It also perfroms better than chance for each subject.
...
*Subject level p-values:*
```{r message=FALSE, warning=FALSE}
lobo.full_null <- rf.null_test(forest.lobo.full, forest.boot.combi.merge, model_type="LOBO", method="actual")
lobo.full_null$p_val
```
#### LOSO Model {-}
```{r message=FALSE, warning=FALSE}
# Then with the full model
...
*Subject level p-values:*
```{r}
loso.full_null <- rf.null_test(forest.loso.full, forest.boot.combi.merge, model_type="LOSO", method="actual")
loso.full_null$p_val
```


### Within Model Testing

Now that all models have been estimated, we will first test the within-model differences in the error rates. This means that for both the LOBO and the LOSO, we test models 1, 2, and 3 against each other. 

#### LOBO Models {.tabset}

We start with the highest error rates, and that is the mood and physiology models (models 1 and 2). We then test the mood against the combined LOBO model, Based on these results, we see that Model 3 > Model 1 > Model 2. So the combination of both the mood and physiology data is the best in these types of models
...
#### LOSO Model {.tabset}
We now do this again for the LOSO models, where we compare the LOSO models 1, 2, and 3 to each other. We againn see the same pattern, with the model using the comined values have the highest accuracy. 
```{r}
...
We will now do a between method test. We will compare the LOBO, LOSO, and the bootstraped estimates against each other. First we combine the model results togehter into a signle dataframe below to make it easier. 
```{r}
...
First, we test whether the mood models perfrom better than the bootstrap counterpart, and then test them empirically against each other. 
...
#### Effect Sizes {-}
```{r}
df_loo.m1 <- df_loo %>% filter(Model=="mood") %>% na.omit()
rstatix::cohens_d(data=df_loo.m1, error ~ Method, hedges.correction = TRUE)
```
...
#### Effect Sizes {-}
```{r}
df_loo.m2 <- df_loo %>% filter(Model=="physio") %>% na.omit()
rstatix::cohens_d(data=df_loo.m2, error ~ Method, hedges.correction = TRUE)
```
...
#### Effect Sizes {-}
```{r}
df_loo.m3 <- df_loo %>% filter(Model=="full") %>% na.omit()
rstatix::cohens_d(data=df_loo.m2, error ~ Method, hedges.correction = TRUE)
```
