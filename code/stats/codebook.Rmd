---
title: "Momentary mood and arousal predict real-life prolonged stress"
...
Before starting, we also load the custom functions written for the study that are found in the GitHub directory.
```{r}
source("functions.R")
```
...
Now that we are done with the data cleaning, we can move on to the analysis.


Using python, we derived physiological arousal features in 10-minute time windows prior to each EMA survey from the raw EPA wristwatch data for each participant. Preprocessing is done through python with the [pyphysio package](https://github.com/MPBA/pyphysio) which has some tools for processing bio-signals. EMA data is not processed during this phase and the raw output from the surveys is loaded into R. Among other things, the EMA and EPA signals contain the following features:
...
- Social Stress
- Mood (Positive and Negative)
- Heart rate (High/Low Frequency, RMSSD, IBI...)
- Skin Conductance (Phasic, Tonic, Magnitude...)
- Skin Temperature (Slope, Mean)
- Accelorometer Data (delta ACC, mean X,Y,Z direction)


Due to privacy issues, only a specific file (and not the full file) with anonymized categorical variables for the program are provided in this code. All data presented can be used for replication of the findings. Generated variables include different suffixes, which each represent an alternative mode of calculating summary statistics. Below is a description of what each suffix stands for:

- c:  Variable is subject mean centered
- z:  Variable is z-transformed at the population level (centered and scaled)
- s:  Variable is Z-transformed and re-scaled to 1-10 for analysis with different model families
- m:  Variable mean in each week for each subject 
- mz: Variable mean from above is z-transformed
- l:  Variable is Temporally Lagged (-1)
- cs: Variable is subject centered, then re-scaled to 1-10
...
### IBI Based Quality {-}
This is a measure of what duration of time the detected heart beats cover. We see that for the mean is 27% of time with beats detected. 
```{r}
...
### SC and ACC {-}
Next we will check if the SC signals are related to the ACC. Basically this is just doing exploratory analysis to double check certain things we already. 
```{r message=FALSE, warning=FALSE}
...
## Trends in EMA
We finally plot the graphs of the EMA items over time in each of the weeks for some exploratory analysis.
```{r message=FALSE, warning=FALSE}
...
It looks like we have a bit of a trend in our data. The closer we are to the exams, the more stress people become. We will need to add this to our models to correct for the trend statistically.
...
Next, we want to investigate the impact of an examination period on mood. Conceptually, mood is more of an outcome of being stressed than a measure of stress.
...
We established changes in subjective stress between the weeks, which has an impact on our mood outcomes. We also want to see if there's a general difference between the physiology variables and the weeks.To do this we apply the same approach for the physiology measures as the other models. We also add measures of skin temperature here however, to make sure that we factor this in. Skin temperature is important as it can be related to sweat or heat, which can also lead to increased arousal. 
...
We next model the skin conductance vs. the stress measures. We expect that in this analysis, we see an expected increase in arousal linked to subjective stress. We do indeed confirm this below. 
...
### Mediate Number
First we checke whether positive mood mediates the effects of week type of the number of responses
```{r}
# Main effect
...
# Mediation analysis
results1 = mediate(model.m=fit.mediator, model.y=fit.dv, treat='week_type', mediator='mood_positive_s', boot=F, sims=5000)
summary(results1)

```

### Mediate Magnitude
Then we check the mediation effects on the magnitude of the SC responses.
```{r}
# Main effect
...
#### Stouffer's Test{-}
We will now test the LOBO model using Mood against the bootstrapped LOBO model with the same variables. We made a function to do this. The function estimates the p-value by comparing it to the permuation test distribution. The function also has the option to estimate the p-value from an assumed normal distribution, but we simply use the actual distribution for this. We additionally compute a combined p-value with this function using Stoufferś method and the poolr package Using this method, we see the LOBO models perfrom significantly better than chance. Yay! So within-subject approach works super well! 
